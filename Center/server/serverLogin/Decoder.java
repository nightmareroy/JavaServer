...
import managerGameClient.GameClient;
...
public class Decoder extends LengthFieldBasedFrameDecoder {
	...
	protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
		...
		case LogicReq:
			...
			Out.debug("recieve:",logicPacket.route);
			
			
			GameClient gameClient=GameClientManager.getInstance().getOnlineClient(logicPacket.packet.channel);
			gameClient.logicDispatcher.addPacket(logicPacket);
			break;
		case LogicRes:
		...
		case LoginReq:
			...
			//如果开启白名单，检测
			if(Config.CenterGameNet.CenterServer.WhiteList.Enable) {
				if(!Arrays.asList(Config.CenterGameNet.CenterServer.WhiteList.List).contains(remoteAddress.getAddress().getHostAddress()))
				{
			...
			GameClientManager.getInstance().registerClient(new GameClient(packet.channel));
...
//		cause.printStackTrace();
		GameClientManager.getInstance().leavingClient(ctx.channel());
		ctx.close();
    }
