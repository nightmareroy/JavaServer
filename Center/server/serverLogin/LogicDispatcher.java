...
private BlockingQueue<LogicPacket> blockingQueue = new LinkedBlockingQueue<LogicPacket>(Config.CenterGameNet.CenterServer.DispatcherQueue.Capacity);
	...
	public void init()
	{
		...
		for (ProtoService protoService : ProtoServiceManager.protoCenterGameServiceMap.values()) {
			...
			try {
				Object object=protoService.handlerClass.newInstance();
				handlerMap.put(protoService.route, object);
				Out.debug(object);;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	{
		if (blockingQueue.size() > Config.CenterGameNet.CenterServer.DispatcherQueue.WarnCount) {
